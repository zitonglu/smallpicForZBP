...
if (!$zbp->CheckPlugin('SmallPIC')) {$zbp->ShowError(48);die();}

$blogtitle='SmallPIC文章特殊图片插件';

if(count($_POST)>0){

	$zbp->Config('SmallPIC')->default_url=$_POST['default_url'];
	$zbp->Config('SmallPIC')->source=$_POST['source'];
	$zbp->Config('SmallPIC')->local_priority=$_POST['local_priority'];	
